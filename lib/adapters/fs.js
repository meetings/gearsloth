...
function initialize(config, callback) {
  ...
  async.series([
    ...
    if (err) {
      var msg = 'Failed to create destination directories';
      log.err(msg, err);
...
function FsAdapter(config) {
  this.path = '/usr/lib/gearsloth/fs-adapter';
  this.db = os.hostname();
  this.poll_timeout = 500; // ms

  if (typeof config.dbopt === 'undefined') return;

  this.path = config.dbopt.path || this.path;
  this.db = config.dbopt.db || this.db;
  this.poll_timeout = config.dbopt.poll_timeout || this.poll_timeout;
}
...
FsAdapter.prototype.updateListenedTask =
function(modified_task, db_state, callback) {
  async.waterfall([
...
FsAdapter.prototype.listen_loop = function() {
  ...
  this.depth_first_traverse_lesser_or_equal_dirs(
    path.join(this.path, 'due'),
    dirs,
    function(leaf_dir) {
      this.randomly_traverse_all_subdirs_for_files(leaf_dir, function(file) {
        var task_file = this.return_task_file_for_due_file(file);
        this.retrieve_task_object_from_file(
          task_file, this.lock_and_handle_due_task.bind(this)
        );
      }.bind(this));
    }.bind(this)
  );
...
FsAdapter.prototype.lock_and_handle_due_task =
function(task_error, original_task) {
  if (! task_error) {
    this.lock_task(original_task, function(lock_error, lock) {
      if (! lock_error && this.listen_runner_task_handler) {
        this.listen_runner_task_handler(
          null, original_task, this.db, { lock: lock }
        );
      }
    }.bind(this));
  }
...
FsAdapter.prototype.generate_id = function() {
  ...
  var id = _.map(pool, function() {
    return pool[Math.floor(Math.random() * pool.length)];
  });
  return id.join('');
};

FsAdapter.prototype.lock_task_with_new_id =
function(task, callback, failure_count) {
  ...
  async.waterfall([
    ...
    if (error || content !== '') {
      this.release_lock(current_lock, function() {
        if (failure_count > 99) {
          return callback(
            new Error('Could not find and lock a unique id file: ' + error)
          );
        }
        else {
...
FsAdapter.prototype.lock_task = function(task, callback) {
  this.lock_task_or_call_error_handler(
    task, callback, function(lock) { callback(null, lock) }
  );
};

FsAdapter.prototype.lock_task_or_call_error_handler =
function(task, error_handler, callback) {
  // TODO remove lock files that are older than 15
  //      seconds to defend against previous crashes

  var lock = {
    id: this.generate_id(),
    file: path.join(this.path, 'lock', task.id.id)
  };
  var opts = {
    encoding: 'utf8',
    flag: 'wx'
  };
  fs.writeFile(lock.file, lock.id, opts, function(error) {
    if (error) {
...
/* Read `file` and suppress all errors. Callback is called with contents
 * of the file or an empty string.
 */
...
FsAdapter.prototype.retrieve_task_object_from_file = function(file, callback) {
  FsAdapter.prototype.read_possibly_missing_file(file, function(err, content) {
    var task;
    if (err) {
      return callback(err);
    }
...
FsAdapter.prototype.store_task_string_for_task =
function(task, string, callback) {
  var file = this.return_task_file_for_task(task);
...
FsAdapter.prototype.store_disabled_task_string_for_task =
function(task, string, callback) {
  var file = this.return_disaled_task_file_for_task(task);
  this.store_task_string_to_file(file, string, callback);
};

FsAdapter.prototype.store_task_string_to_file =
function(file, string, callback) {
  async.series([
...
FsAdapter.prototype.return_task_file_for_task_id =
function(id, alternative_dir) {
  var dir = alternative_dir ? alternative_dir : 'task';
  var id_match = /(..)(..)(..)(..)(..)(..)(.*)/.exec(id);

  var task_dir = path.join(
    this.path, dir, path.join.apply(null, id_match.splice(1, 6))
  );
...
FsAdapter.prototype.return_date_path_strings = function(date, round_up) {
  ...
  var date_parts = [
    rounded_date.getUTCFullYear(), rounded_date.getUTCMonth() + 1,
    rounded_date.getUTCDate(),     rounded_date.getUTCHours(),
    rounded_date.getUTCMinutes(),  rounded_date.getUTCSeconds()
  ];
  return _.map(date_parts, function(part) { return (part < 10 ? '0' : '') + part.toString() });
};

FsAdapter.prototype.return_due_file_for_task = function(task) {
  var date_path_str = this.return_date_path_strings(new Date(task.at), true);
  var date_path = path.join.apply(null, date_path_str);
  var id_match = /(..)(..)(..)(.*)/.exec(task.id.id);

  var due_dir = path.join(
    this.path, 'due', date_path, path.join.apply(null, id_match.splice(1, 3))
  );

  return path.join(due_dir, task.id.id);
};

FsAdapter.prototype.depth_first_traverse_lesser_or_equal_dirs =
function(start_path, remaining_dirs, leaf_callback) {
      ...
      fs.readdir(start_path, function(error, files) {
        if (! error) {
          ...
          _.each(files, function(file) {
            try {
              if (parseInt(file, 10) === parseInt(current_dir, 10)) {
                FsAdapter.prototype.depth_first_traverse_lesser_or_equal_dirs(
                  path.join(start_path, file),
                  next_remaining_dirs,
                  leaf_callback
                );
              }
              if (parseInt(file, 10) < parseInt(current_dir, 10)) {
                FsAdapter.prototype.depth_first_traverse_lesser_or_equal_dirs(
                  path.join(start_path, file), [ '9999' ], leaf_callback
                );
              }
            }
            catch (e) {
              log.err('failed to parsed directory name:', file, e.toString());
            }
          });
        }
      });
...
FsAdapter.prototype.randomly_traverse_all_subdirs_for_files =
function(start_path, file_callback) {
  fs.stat(start_path, function(error, stats) {
    if (! error) {
      if (stats.isDirectory()) {
        fs.readdir(start_path, function(error, files) {
          if (! error) {
            _.each(files, function(file) {
              FsAdapter.prototype.randomly_traverse_all_subdirs_for_files(
                path.join(start_path, file), file_callback
              );
            });
          }
        });
...
FsAdapter.prototype.generate_result_callback_that_releases_lock =
...
FsAdapter.prototype.generate_result_callback_that_returns_nothing =
function(callback) {
  return function(error) {
    callback(error);
  }
...
FsAdapter.prototype.testInterfaceGatherEnabledJobMetaList = function(callback) {
  var task_list = [];
  var domain = this.db;
  this.traverse_subdirs_append_tasks_from_files(
    path.join(this.path, 'task'),
    task_list,
    function(error) {
      var map_helper = function(i) { return { job: i, domain: domain }; };
      var job_meta_list = _.map(task_list, map_helper);
      callback(error, job_meta_list);
    }
  );
};

FsAdapter.prototype.traverse_subdirs_append_tasks_from_files =
function(start_path, task_list, done) {
  fs.stat(start_path, function(error, stats) {
    if (! error) {
      if (stats.isDirectory()) {
        fs.readdir(start_path, function(error, files) {
          if (error) {
            ...
            async.each(
              ...
              function(file, callback) {
                FsAdapter.prototype.traverse_subdirs_append_tasks_from_files(
                  path.join(start_path, file), task_list, callback
                );
              },
              function(error) {
                done(error);
        ...
        FsAdapter.prototype.retrieve_task_object_from_file(
          start_path,
          function(error, task) {
            if (error) {
              return done(error);
