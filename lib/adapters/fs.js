  ...
  async.waterfall([
    ...
    if (error || content !== '') {
      this.release_lock(current_lock, function() {
        if (failure_count > 99) {
          return callback(new Error('Could not find and lock a unique id file: ' + error));
        }
        else {
...
FsAdapter.prototype.return_date_path_strings = function(date, round_up) {
  ...
  var date_parts = [
    rounded_date.getUTCFullYear(), rounded_date.getUTCMonth() + 1, rounded_date.getUTCDate(),
        ...
        if (! error) {
          ...
          _.each(files, function(file) {
            try {
              ...
              if (parseInt(file, 10) < parseInt(current_dir, 10)) {
                FsAdapter.prototype.depth_first_traverse_lesser_or_equal_dirs(path.join(start_path, file), [ '9999' ], leaf_callback);
              }
            }
            catch (e) {
