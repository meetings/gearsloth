...
// database query strings
  var table_init = "CREATE TABLE IF NOT EXISTS "+table_name+" ( \
           ...
           func_name BLOB NOT NULL, \
           payload BLOB, \
           strategy BLOB, \
           strategy_options TEXT, \
           status VARCHAR(10))";
           
  var insertion = "INSERT INTO "+table_name+"(\
           at, func_name, payload, strategy, strategy_options, status) \
           VALUES (?, ?, ?, ?, ?, ?)";
           
  var selection_new = "SELECT * FROM "+table_name+" WHERE at < DATETIME() \
                   AND status = '"+status.NEW+"'";
                   
  var selection_task = "SELECT * FROM "+table_name+" WHERE id = ?";
  
  var update_task = "UPDATE "+table_name+" SET status = ? WHERE id = ?";
 ...
 */
  function saveTask(task, callback) {
    database.serialize(function() {
      ...
      try {
        var strategy_options = JSON.stringify(task.strategy_options);
      } catch (err) {
        callback(err);
        return;
      }

      database.run(insertion, task.at, task.func_name, task.payload, task.strategy, strategy_options, status.NEW, function(error) {
        if (error) {
          callback(error);
        }
        callback();
        ...
        database.all(selection_new, function(error, rows) {
         if (error) {
          ...
          rows.forEach(function (row) {
            callback(error, row.id, row.strategy);
          });
