...
// CONFIGURATION
  ...
  var db_id = config.db_opt.db_name ? config.db_opt.db_name : ':memory:';
 ...
 */
  ...
  function updateTask(task, callback) {
    database.serialize(function() {
      var expiry_time = getExpiryTime(task);
      var update_task = "UPDATE "+table_name+" SET task = ? at = ? WHERE id = ?";
      database.run(update_task, task, expiry_time, task.id.task_id, function(error) {
        if (error) {
          callback(error);
        }
        callback();
