...
var sql = {
  ...
  listen_task:
    "SELECT * FROM ? WHERE (datetime('now') >= datetime(%s.at)) AND disabled = 0",
  update_task:
    "UPDATE ? SET at = datetime('now', '%s seconds') WHERE %s.id = ?"
};
...
SQLiteAdapter.prototype.init_db = function(err) {
  ...
  this.db.run(sql.init_table, this.table, function(err) {
    if (err) {
      log.err('failed to initialize table', this.table);
    }
    log.debug('initialized table', this.table);
  });
};
...
SQLiteAdapter.prototype.saveTask = function(task, callback) {
  var opts = [this.table, JSON.stringify(task)];
...
SQLiteAdapter.prototype.listenTask = function(callback) {
  var $ = this;
  var listenQ = fmt(sql.listen_task, this.table);
  var updateQ = fmt(sql.update_task, this.default_timeout, this.table);
