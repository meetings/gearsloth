...
function SQLiteAdapter(config) {
  ...
  /// ?? this.db_id = config.dbopt.db_name || 'DelayedTasks.sqlite';
  /// ?? this.pre_poll = config.dbopt.pre_poll || 0;

  this.domain = config.dbopt.domain || _id;
  this.table_name = config.dbopt.table_name || 'DelayedTasks';
  ...
  this.db = new sqlite3.Database('DelayedTasks.sqlite', this.init_db);
}

SQLiteAdapter.prototype.init_db = function(err) {
  if (err) {
    log.err('failed to create database instance');
    log.err(util.inspect(err));
  }

  this.db.run(sql.init_table, this.table_name, function(err) {
    if (err) {
      log.err('failed to initialize table', this.table_name);
    }
    log.debug('initialized table', this.table_name);
  });
};
...
SQLiteAdapter.prototype.saveTask = function(task, callback) {
  var opts = [this.table_name, JSON.stringify(task)];

  this.db.run(this.get_save_task_sql, opts, function(err) {
    if (err) {
...
SQLiteAdapter.prototype.listenTask = function(runner_task_handler) {
};

SQLiteAdapter.prototype.updateListenedTask = function(modified_task, db_state, callback) {
};

SQLiteAdapter.prototype.disableListenedTask = function(task, db_state, callback) {
};

SQLiteAdapter.prototype.completeTask = function(task, callback) {
};

SQLiteAdapter.prototype.get_save_task_sql = function(task) {
  var after = parseInt(task.after, 10);

  if (!isNaN(after)) {
    return util.format(sql.save_task_after, after);
  }

  if (task.at && task.at.toISOString) {
    return util.format(sql.save_task_at, task.at.toISOString());
  }

  return util.format(sql.save_task_at (new Date).toISOString());
};
