...
// CONFIGURATION
  ...
  var poll_timeout = config.dbopt.poll_timeout ? config.dbopt.poll_timeout : 1000;
  
  var pre_poll = config.dbopt.pre_poll ? config.dbopt.pre_poll : 0;
 ...
 */
  function listenTask(callback) {
    ...
    function poll() {
      database.serialize(function() {
        var selection_new = "SELECT * FROM "+table_name
          +" WHERE datetime('now') >= datetime("+table_name+".at, '-"+pre_poll+" seconds')";

        var update_selection = "UPDATE "
        +table_name+" SET at = datetime('now', '+1000 seconds')"
        +" WHERE ("+table_name+".id = ?) "
        +"AND (datetime('now') >= datetime("+table_name+".at, '-"+pre_poll+" seconds'))";
