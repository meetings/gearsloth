...
// CONFIGURATION
  config = config ? config : {db_opt:{}};
  config.db_opt = config.db_opt ? config.db_opt : {};
  
  var table_name = config.db_opt.table_name ? config.db_opt.table_name : "DelayedTasks";
  
  var db_id = config.db_opt.db_name ? config.db_opt.db_name : "/tmp/DelayedTasks.sqlite";
  
  var poll_timeout = config.db_opt.poll_timeout ? config.db_opt.poll_timeout : 1000;
  
  var pre_poll = config.db_opt.pre_poll ? config.db_opt.pre_poll : 0;
 ...
 */
  function listenTask(callback) {
    ...
    function poll() {
      database.serialize(function() {
        var selection_new = "SELECT * FROM "+table_name+" WHERE datetime('now') >= datetime("+table_name+".at, '-"+pre_poll+" seconds')";
