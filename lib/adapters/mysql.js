...
function initializeTables(adapter, callback) {
  var create_gearsloth =
    'CREATE TABLE IF NOT EXISTS gearsloth (' +
  ...
  var create_gearsloth_disabled =
    'CREATE TABLE IF NOT EXISTS gearsloth_disabled (' +
    ' id         INT NOT NULL AUTO_INCREMENT PRIMARY KEY,' +
    ' at         DATETIME,' +
    ' task       TEXT,' +
    ' INDEX      at (at)' +
...
var default_server_config = {
  timezone: 'Z', // save everything to database in UTC to ensure compatibility
  database: 'gearsloth'
};

function MySQLMultimaster(config) {
  // Assume single-server configuration
  if (!(config.master && config.slave)) {
    config = merge(default_server_config, config);
    config = {
      master: config,
      slave: config
    };
  } else {
    config.master = merge(default_server_config, config.master);
    config.slave = merge(default_server_config, config.slave);
  }
    ...
    function selectFromSlave(cb) {
      slave.query(sql, where, function(err, result) {
        ...
        if (isSaved) {
          ...
          var msg = 'Slave not updated, id: ' + inserted_id;
          setTimeout(cb, 100, new Error(msg));
        }
      });
    }
  }
