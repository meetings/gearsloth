...
MySqlAdapter.prototype.grabTask = function(id, callback) {
  ...
  var toka_sql = 'UPDATE gearsloth SET status = ?status.pending? WHERE id = ?';

  this._connection.beginTransaction(function(err) {
    if (err) callback(err);

    this._connection.query(eka_sql, params, function(err, result) {
      if (err) {
        this._connection.rollback();
      }

      this._connection.query(toka_sql, params, function(err, result) {
        if (err) {
          this._connection.rollback();
        }
