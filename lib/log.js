...
function logWithProperFormatting( streamFunction, level, originalArguments ) {
  ...
  var tokens = ( verbose > 0 ) ? [ level ] : [];

  // first element of `originalArguments` is treated as a source component and is
  // postfixed with a colon (:)
  tokens.push( originalArguments[0] + ':' );

  tokens.push( Array.prototype.splice.call(originalArguments, 1) );

  streamFunction.apply( undefined, tokens );
}
