...
var inject_job_and_wait_for_completion =
exports.inject_job_and_wait_for_completion =
function(adapter, job, callback) {
  Injector.prototype.prepare_task_for_injection(job);
  adapter.saveTask(job, function( err, task_id ) { callback( err ) } );
}

exports.async_inject_job_and_wait_for_completion = function(adapter, job) {
  return function(callback) {
    inject_job_and_wait_for_completion(adapter, job, callback);
  }
}
...
var gather_enabled_job_meta_list = exports.gather_enabled_job_meta_list =
...
exports.async_gather_enabled_job_meta_list = function(adapter) {
  return function(callback) {
    gather_enabled_job_meta_list(adapter, callback);
  }
};

var gather_enabled_job_list = exports.gather_enabled_job_list =
function(adapter, callback) {
