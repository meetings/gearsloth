...
function Multiserver(servers, create_connection, component_name) {
  ...
  this.err = log.err.bind(null, this.component_name);
  this.info = log.info.bind(null, this.component_name);
  this.debug = log.debug.bind(null, this.component_name);
}
...
Multiserver.prototype.disconnect = function() {
  this.info('disconnecting all servers');
...
Multiserver.prototype._emitDisconnect = function() {
  this.info('all servers disconnected');
  this.connected = false;
...
Multiserver.prototype._registerListeners = function() {
  ...
  this._connections.forEach(function(connection, index) {
    ...
    connection.reconnecter.on('disconnect', function(err) {
      that.info('disconnected from', that._serverString(index));
      if (err) {
        that.debug(util.inspect(err));
      }
      else {
    ...
    connection.reconnecter.on('error', function(err) {
      that.err('received error from', that._serverString(index));
      if (err) {
        that.debug(util.inspect(err));
      }
    });
