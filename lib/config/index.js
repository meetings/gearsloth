 ...
 * `.controllerpath {String}`:    Resolved controller module path.
 *
 * @param {Object} conf
...
var initializeDb = exports.initializeDb = function(conf, callback) {
  // initialize db
  var dbmodule = conf.db.match('/') ?
      path.normalize(path.resolve(conf.db)) :
      '../adapters/' + conf.db;
  conf.controllerpath = conf.controllername.match('/') ?
      
    path.normalize(
      path.resolve('../controllers/' + conf.controllername));
  console.log(conf.controllerpath);
  try {
  ...
  try {
    ...
    throw new Error('Cannot find controller "' + conf.controllerpath + '"');
  }
