...
var initialize = exports.initialize = function(argv) {
  ...
  if (cmdopt.file) {
    ...
    for (var i = 0; i < defaults.conf_paths.length; ++i) {
      if (fs.existsSync(defaults.conf_file)) {
...
var initializeDb = exports.initializeDb = function(conf, callback) {
  conf.dbmodulepath = resolvePath('../adapters', conf.db);
  try {
    require.resolve(conf.dbmodulepath)
  } catch (e) {
    throw new Error('Cannot find database adapter "' + conf.db + '"');
  }

  conf.dbmodule = require(conf.dbmodulepath);
...
/** 
 * Resolve controller path and name and augment the config.
 *
 * @param {Object} conf
 * @return {Object}
 */

var resolveController = exports.resolveController = function(conf) {
  conf.controllerpath = resolvePath('../controllers', conf.controllername);
  try {
    require.resolve(conf.controllerpath);
  } catch (e) {
    throw new Error('Cannot find controller "' + conf.controllername + '"');
  }
  conf.controllername = path.basename(conf.controllername, '.js')
    + 'Controller';
...
function resolvePath(base, name) {
  return path.normalize(name.match('/') ?
      path.resolve(name) :
      path.join(__dirname, base, name));
}
  ...
  for (var i = 2; i < argv.length; ++i) {
    ...
    // parse long options
      ...
      if (argv[i] === '--help')
        ...
        ret.file = match[1];
      else if (match = argv[i].match(/^--controllername=(.+)$/))
