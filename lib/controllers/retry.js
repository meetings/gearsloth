...
Retry.prototype.workHandler = function(task, worker) {
  if(!this._client.connected) {
    log.err(this.component_name, 'No reachable job servers');
  }
  ...
  function submitRetryJob() {
    if (task.retry_count === 0) {
      ...
      log.info(that.component_name, 'Giving up:', taskString(task));
    ...
    log.info(that.component_name, 'Trying:', taskString(task));
    try {
      that._client.submitJob(task.func_name, task.payload)
      .on('complete', function() {
        if (done) return;
        done = true;
        log.info(that.component_name, 'Completed:', taskString(task));
        that.submitDone(task, timeout_handle);
      })
      .on('fail', function() {
        if (done) return;
        done = true;
        log.info(that.component_name, 'Failed:', taskString(task));
        that.submitDone(task, timeout_handle);
      });
    } catch (err) {
