...
function Injector(conf) {
  ...
  this._default_controller = defaults.controllerfunc(conf);
  ...
  this._worker = new MultiserverWorker(conf.servers, 'submitJobDelayed',
    function(payload, worker) {
      ...
      log.info(that.component_name, 'Received a task for',
          (task.controller || that._default_controller) + '/' + task.func_name);
