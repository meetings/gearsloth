...
chai.should();
suite('Docker load test', function() {
  suite('Docker test: load test with two components', function(){
    ...
    var test_func1_200 = function(done, payload, worker){
        payload = payload.toString();
        expect(payload).to.equal(simple_task.payload);
        ++task_counter;
    ...
    var test_func2_200 = function(done, payload, worker){
      payload = payload.toString();
      expect(payload).to.equal(simple_task.payload);
      ++task_counter;
      if (task_counter >= total_tasks) {
  ...
  suite('Docker test: load test with two components', function(){
    ...
    test('with 200 tasks submitted simultaneously, immediate tasks are executed', function(done) {
      this.timeout(30000);
      ...
      async.series([
        function(callback_outer) {
          async.parallel([
            function(callback) {
              worker1 = new MultiserverWorker(gearslothd_config.servers, 'test', test_func1_200.bind(null, done));
              worker1.on('connect', function() {
                callback();
              });
            },
            function(callback) {
              worker2 = new MultiserverWorker(gearslothd_config.servers, 'test', test_func2_200.bind(null, done));
              worker2.on('connect', function() {
                callback();
              });
            },
        ...
        function(callback_outer) {
          async.times(total_tasks, function() {
            client.submitJob('submitJobDelayed', JSON.stringify(simple_task));
          }, callback_outer);
        }
      ]);
